...
void parseCommands(char *ptrBufferReception){
	...
	// Se imprimen los datos medidos en el mapeo
	else if (strcmp(cmd, "getData") == 0){
		writeMsg(&handlerUSART1, "CMD: getData\n");
		getRangingData();
	}
	// Se imprimen las salidas del Encoder mientras el motor hace un movimiento específico
	...
	// Se hace un movimiento del motor una cantidad específica de pasos en cierta dirección
	...
	// Se hace un movimiento del motor una cantidad específica de grados en cierta dirección
